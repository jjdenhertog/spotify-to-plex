...
export default function PlaylistItemSettings(props) {
    ...
    const onEditLabelChange = useCallback((e) => {
    ...
    const onEditLabelChipClick = useCallback((e) => {
    ...
    return (<Modal open>
        <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', maxWidth: 500, bgcolor: 'background.paper', p: 3, borderRadius: 1 }}>
            ...
            {!loading && <>
                ...
                <Box sx={{ mt: 1 }}>
                    {labels.map(label => (<Chip variant="outlined" size="small" sx={{ mr: .5, mb: .5 }} key={label} data-label={label} onClick={onEditLabelChipClick} label={label}/>))}
                </Box>
...
//# sourceMappingURL=PlaylistSyncSettings.jsx.map