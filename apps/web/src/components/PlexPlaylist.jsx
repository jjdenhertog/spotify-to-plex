...
export default function PlexPlaylist(props) {
    ...
    ///////////////////////////////////////////////
    const [plexPlaylist, setPlexPlaylist] = useState();
    useEffect(() => {
        if (!playlist)
            return;
        setTotalPages(Math.ceil(playlist.tracks.length / pageSize));
        errorBoundary(async () => {
            const playlistResult = await axios.get(`/api/playlists/${playlist.id}`);
            setPage(0);
    ...
    const loadPlaylistTracks = useCallback(async (items) => {
        ...
        while (curPage < pages) {
            ...
            try {
                const result = await axios.post('/api/plex/tracks', {
                    items: tracksToLoad,
                    type: playlist.type,
                    fast
        ...
        errorBoundary(async () => {
            ...
            switch (playlist.type) {
                ...
                case "spotify-playlist":
                    // Load cached tracks
                    const cachedResult = await axios.post('/api/plex/cached', {
                        items: tracks,
                    }, { signal: abortController.current.signal });
    ...
    ///////////////////////////////////
    const onSetSongIndex = useCallback((artist, track, idx) => {
    ...
    const onPlaylistNameChange = useCallback((e) => {
        if (e.target.value)
    ...
    const onPutPlaylistClick = useCallback(() => {
        ...
        errorBoundary(async () => {
            if (plexPlaylist) {
                await axios.put(`/api/playlists/${playlist.id}`, data);
                enqueueSnackbar("Playlist updated");
            }
            else {
                const result = await axios.post('/api/playlists', data);
...
//# sourceMappingURL=PlexPlaylist.jsx.map