...
export default function ManagePlaylistItem(props) {
    ...
    const onEditItemClick = useCallback((e) => {
        e.preventDefault();
        e.stopPropagation();
        setEditItem(true);
    }, []);
    const onCloseEditItem = useCallback((reload) => {
        setEditItem(false);
        if (reload)
            reloadSavedItems();
    }, [reloadSavedItems]);
    ...
    // Delete Item
    ///////////////////////////////
    const onDeleteItemClick = useCallback((e) => {
        e.preventDefault();
        e.stopPropagation();
        errorBoundary(async () => {
            await confirm({
            ...
            await axios.delete(`/api/saved-items?id=${item.id}`);
            reloadSavedItems();
                    ...
                    <Box sx={{ minWidth: 0 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: .5, alignItems: 'center' }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                {item.type !== 'plex-media' && (<Tooltip title={item.sync ?
...
//# sourceMappingURL=ManagePlaylistItem.jsx.map