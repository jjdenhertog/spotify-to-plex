...
export default function ExportMissingTracks(props) {
    const { onClose, tracks, playlist } = props;
    const { type } = playlist;
    ///////////////////////////////////////////////
    // Tidal Tracks
    ///////////////////////////////////////////////=
    const [tidalTracks, setTidalTracks] = useState([]);
    ///////////////////////////////////////////////
    // Pagination
    ///////////////////////////////////////////////
    const pageSize = 10;
    ...
    const loadTidalTracks = useCallback(async (items) => {
        const pageSize = 1;
        let curPage = 0;
        ...
        while (curPage < pages) {
            ...
            try {
                const result = await axios.post('/api/tidal', {
                    type,
                    items: tracksToLoad,
    ...
    useEffect(() => {
        errorBoundary(async () => {
            const validResult = await axios.get('/api/tidal/valid');
            ...
            setCanUseTidal(true);
            // Load cached tracks
            const cachedResult = await axios.post('/api/tidal/cached', {
                items: tracks,
            });
...
//# sourceMappingURL=ExportMissingTracks.jsx.map