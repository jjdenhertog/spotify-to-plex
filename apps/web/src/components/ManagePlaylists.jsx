...
export default function ManagePlaylists() {
    ...
    useEffect(() => {
        errorBoundary(async () => {
            const result = await axios.get(`/api/saved-items`);
    ...
    // Add Item
    ///////////////////////////////
    const onChangeSpotifyInput = useCallback((e) => {
        setSearchInput(e.currentTarget.value);
    }, []);
    const onAddPlaylistClick = useCallback(() => {
        errorBoundary(async () => {
            setGenerating(true);
            const result = await axios.post(`/api/saved-items`, {
                search: searchInput
            });
            setItems(result.data);
            setSearchInput('');
    ...
    // Selecting multiple
    //////////////////////////////////
    const [editMultipleItems, setEditMultipleItems] = useState([]);
    const onEditSettingsClick = useCallback(() => {
        setEditMultipleItems(items.filter(item => selectedItems.has(item.id)));
    }, [items, selectedItems]);
    const onCloseMultipleEditItem = useCallback((reload) => {
            ...
            :
                ...
                <Paper elevation={0} sx={{ p: 2, mb: 2, bgcolor: 'action.hover' }}>
                    {Object.keys(groupedItems).length > 0 &&
                        <>
                            {Object.entries(groupedItems).map(([label, groupItems]) => (<Box key={label} sx={{ mb: 4 }}>
                                    <Typography variant="h6" sx={{ mb: 2 }}>{label}</Typography>
                                    <Grid container spacing={2}>
                                        {groupItems.map(item => (<Grid size={{ xs: 6, sm: 4, md: 4, lg: 3 }} key={item.id}>
...
//# sourceMappingURL=ManagePlaylists.jsx.map