...
jest.mock('../../../src/components/MonacoJsonEditor', () => {
  return React.forwardRef<any, any>(({ value, onChange, error }, ref) => {
    ...
    return (
      <div data-testid="monaco-editor">
        ...
        />
        {error && <div data-testid="monaco-error">{error}</div>}
      </div>
    );
  });
});
...
jest.mock('../../../src/components/TableEditor', () => {
  return function TableEditor({ filters, onChange }: { filters: MatchFilterRule[], onChange: (filters: MatchFilterRule[]) => void }) {
    return (
...
jest.mock('../../../src/components/EditorHeader', () => {
  return function EditorHeader({ 
    ...
    title: string;
    viewMode: 'ui' | 'json';
    onViewModeChange: (event: any, mode: 'ui' | 'json' | null) => void;
    onSave: () => void;
    onReset: () => void;
    disabled?: boolean;
  }) {
