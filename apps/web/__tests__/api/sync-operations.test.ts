    ...
    describe('Performance and reliability', () => {
        ...
        it('should handle memory-intensive sync operations', async () => {
            ...
            mockSyncPlaylists.mockImplementation(async () => {
                ...
                const largeArray = Array.from({length: 100_000}).fill(null)
                    .map((_, i) => ({
                        id: i,
                        name: `Playlist ${i}`,
                        tracks: Array.from({length: 50}).fill(null)
                            .map((_, j) => `Track ${j}`),
                        processed: false
                    }));
        ...
        it('should handle partial sync failures gracefully', async () => {
            ...
            mockSyncAlbums.mockImplementation(() => {
                ...
                // Some albums failed to sync, but operation continues (removed console.warn for testing)
